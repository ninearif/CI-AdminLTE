...
class Leave extends Admin_Controller {
	...
	public function index()
	{
        ...
        else
        {
            ...
            /* Load Template */
            $this->template->admin_render('admin/leave/index', $this->data);
        }
	}
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('last_name', 'lang:leave_lastname', 'required');
		$this->form_validation->set_rules('email', 'lang:leave_email', 'required|valid_email|is_unique['.$tables['users'].'.email]');
		$this->form_validation->set_rules('phone', 'lang:leave_phone', 'required');
		$this->form_validation->set_rules('company', 'lang:leave_company', 'required');
		$this->form_validation->set_rules('password', 'lang:leave_password', 'required|min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']|max_length[' . $this->config->item('max_password_length', 'ion_auth') . ']|matches[password_confirm]');
		$this->form_validation->set_rules('password_confirm', 'lang:leave_password_confirm', 'required');
	...
	public function deactivate($id = NULL)
	{
		...
		if ($this->form_validation->run() === FALSE)
		{
            ...
            /* Load Template */
            $this->template->admin_render('admin/leave/deactivate', $this->data);
		}
		else
		{
	...
	{
        /* Breadcrumbs */
        $this->breadcrumbs->unshift(2, lang('menu_leave_profile'), 'admin/groups/profile');
        $this->data['breadcrumb'] = $this->breadcrumbs->show();
